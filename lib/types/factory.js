...
function Factory() {
  var types = {
    ...
    "function": new ObjectMetadata('function', { validates: true}),
    custom: []
  };
  Factory.ioc.register('object').as.value();
  Factory.ioc.register('types').from.value(types);
}
...
var get = function (name) {
  var self = this
    , key = helpers.strings.toKey(name);
  if (self.can(key)) {
